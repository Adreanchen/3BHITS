...
Für das Projekt wurde eine Netzwerk-Infrastruktur entworfen, die auf einer VLAN-Trennung basiert. Das Routing zwischen den VLANs übernimmt ein zentraler Cisco 2911 Router. Gemäß der Vorgabe wurde für die Clients das Netz **10.5.0.0/23** verwendet. Der externe Webserver wurde in einem separaten Subnetz isoliert.
...
| -        | Webserver  | 192.168.5.0 | 192.168.5.1 | 255.255.255.0   | /24      |

------

## 2. Netzwerk-Topologie

Die Infrastruktur nutzt eine Stern-Topologie. Der Cisco 2911 Router dient als Gateway für alle Netze. Der Core-Switch (3650) verteilt die VLANs per Trunk an die Stockwerk-Switches. Der Webserver ist zur Sicherheit direkt an ein zweites Interface des Routers angebunden.

<img src="topologie.png" alt="bild von topologie">

------

## 3. DHCP- & DNS-Konfiguration

Ein zentraler DHCP-Server im Management-VLAN versorgt alle Endgeräte. Da sich der Server in einem anderen Subnetz befindet, wurde auf den Subinterfaces des Routers ein DHCP-Relay-Agent (`ip helper-address`) konfiguriert. Zusätzlich wurde ein DNS-Server bereitgestellt, der Namensauflösungen im Netzwerk verarbeitet.

### Wichtige Server-Daten:

- **DHCP-Server:** 10.5.0.230 (Gateway: 10.5.0.225)
- **DNS-Server:** 10.5.0.231 (Gateway: 10.5.0.225)
- **Webserver:** 192.168.5.3 (Gateway: 192.168.5.1)
...
Der Router übernimmt das Routing mittels Subinterfaces am Port G0/0. Port G0/1 wird für das externe Webserver-Netz genutzt.
...
Der Core-Switch arbeitet als reiner Layer-2-Verteiler.
...
### 4.3 Stockwerk-Switches (Beispiel Stockwerk EG)

*Konfiguration der Access-Ports für die Endgeräte.*
